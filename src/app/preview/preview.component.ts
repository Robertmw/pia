...
export class PreviewComponent implements OnInit {
  ...
  ngAfterViewChecked() {
    document.querySelector(
      "angular2csv > button"
    ).innerHTML = this._translateService.instant("summary.download_csv");
  }
  ...
  async generateZip(element) {
    setTimeout(() => {
      ...
      this.addAttachmentsToZip(zip).then((zip2: any) => {
        ...
        var fileTitle = this._translateService.instant(
          "summary.action_plan.title"
        );
        const blob = this.exportCSVFile(
          headers,
          csvContentFormatted,
          fileTitle
        );
        ...
        evaluation = {
          ...
          gauges: {
            ...
            seriousness: evaluationModel.gauges
              ...
              : null,
            likelihood: evaluationModel.gauges
              ? evaluationModel.gauges.y
              : null,
          },
        };
      }
  ...
  toggleIndividualExports() {
    const exportLinksList = document.querySelector(
      ".pia-fullPreviewBlock-exports-individual-links"
    );
    const individualExports = document.querySelector(
      ".pia-fullPreviewBlock-exports-individual"
    );
